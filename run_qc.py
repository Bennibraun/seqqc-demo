...
def analyze_sample(row):
    ...
    # BUG: Not handling potential validation errors
...
def run_quality_control(input_file, output_dir):
    ...
    # Analyze each sample
    results = []
    # BUG: Range is wrong - should be range(len(df))
    # This skips the last sample
    ...
    # BUG: Outlier indices refer to results_df, but we're missing last sample
    # So indices are off by context of original data
    ...
    if 'group' in df.columns:
        ...
        # BUG: Using wrong dataframe - should use results_df indices
        group1_quality = df[df['group'] == 'control']['quality_scores']
        group2_quality = df[df['group'] == 'treatment']['quality_scores']

        # BUG: quality_scores is a string, need to convert
        # This will crash or give wrong results
    ...
    print(f"Total samples analyzed: {len(results_df)}")  # BUG: Wrong count
